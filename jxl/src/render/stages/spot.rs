...
mod test {
    ...
    fn border_pixels_processed() -> Result<()> {
        ...
        if has_border_support {
            ...
            println!(
                "✅ Border pixel processing is working - output size: {:?}",
                output_with_xextra[0].as_rect().size()
            );
            return Ok(());
        }
    ...
    fn variable_channel_count_panics() -> Result<()> {
        ...
        match result {
            ...
            Err(e) => {
                panic!(
                    "Expected success with robust implementation but got error: {}",
                    e
                );
            }
        }
    ...
    fn border_pixel_values_correct() -> Result<()> {
        ...
        input_s
            .as_rect_mut()
            .row(0)
            .copy_from_slice(&[1.0, 1.0, 1.0]); // Full spot coverage
        ...
        input_s2
            .as_rect_mut()
            .row(0)
            .copy_from_slice(&[0.5, 0.5, 0.5]); // Partial coverage
    ...
    fn edge_pixels_match_libjxl_golden() -> Result<()> {
        ...
        input_r
            .as_rect_mut()
            .row(0)
            .copy_from_slice(&[0.1, 0.5, 0.9]); // Edge: 0.1, 0.9
        input_g
            .as_rect_mut()
            .row(0)
            .copy_from_slice(&[0.1, 0.5, 0.9]);
        input_b
            .as_rect_mut()
            .row(0)
            .copy_from_slice(&[0.1, 0.5, 0.9]);
        input_s
            .as_rect_mut()
            .row(0)
            .copy_from_slice(&[0.8, 0.8, 0.8]); // Partial spot coverage
        ...
        println!(
            "Output values: [{:.3}, {:.3}, {:.3}]",
            actual_left, actual_center, actual_right
        );
        println!(
            "Expected: [{:.3}, {:.3}, {:.3}]",
            expected_left, expected_center, expected_right
        );
        println!(
            "Border values in output: left={:.3}, right={:.3}",
            output_row[0], output_row[4]
        );
    ...
    fn border_indexing_bug_demonstration() -> Result<()> {
        ...
        for idx in 0usize..(xsize + 2) {
            ...
            println!(
                "  idx={} ({:>12}) -> OLD: output_idx={}, NEW: {}",
                idx, pixel_type, old_output_idx, new_behavior
            );
        }
        ...
        input_r
            .as_rect_mut()
            .row(0)
            .copy_from_slice(&[0.0, 0.5, 1.0]); // Edge: 0.0, 1.0
        input_g
            .as_rect_mut()
            .row(0)
        ...
        input_s
            .as_rect_mut()
            .row(0)
            .copy_from_slice(&[0.0, 0.0, 0.0]); // No spot effect
        ...
        println!(
            "Actual pixel values: [{:.3}, {:.3}, {:.3}]",
            actual_pixels[0], actual_pixels[1], actual_pixels[2]
        );
    ...
    fn multiple_spot_colors_with_borders() -> Result<()> {
        ...
        input_g
            .as_rect_mut()
            .row(0)
            .copy_from_slice(&[0.2, 0.6, 0.8]);
        input_b
            .as_rect_mut()
            .row(0)
            .copy_from_slice(&[0.4, 0.7, 0.6]);
        ...
        println!(
            "Pixel 0 (unchanged): [{:.3}, {:.3}, {:.3}]",
            final_r[0], final_g[0], final_b[0]
        );
        println!(
            "Pixel 1 (magenta): [{:.3}, {:.3}, {:.3}]",
            final_r[1], final_g[1], final_b[1]
        );
        println!(
            "Pixel 2 (yellow): [{:.3}, {:.3}, {:.3}]",
            final_r[2], final_g[2], final_b[2]
        );
